...
import connectionTransformer, {edgeTransformer} from './connectionTransformer';

describe("connectionTransformer", () => {
  it("returns valid connection from empty entity list", () => {
    // given
    const entities = [];
    ...
    // when
    const got = connectionTransformer(entities, 0);
    // then
    expect(got).toEqual(expectedConnection);
  });

  it("returns valid connection", () => {
    // given
    const entities = [
      new Song({
      ...
      new Song({
        ...
        artist: "Astrometrics",
        duration: 0,
        genre: "Rock, Indie-Rock, Space-Rock, Chip Music",
    ...
    const expectedEdges = map(edgeTransformer, entities);
    const expectedConnection = {
      totalCount: 3,
      edges: expectedEdges,
      pageInfo: {
    ...
    // when
    const got = connectionTransformer(entities, 3);
    // then
    expect(got.totalCount).toEqual(expectedConnection.totalCount);
    expect(got.pageInfo).toEqual(expectedConnection.pageInfo);
